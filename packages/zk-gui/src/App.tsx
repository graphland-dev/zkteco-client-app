...
function DeviceInfoPanel({
  ...
  info: NonNullable<ConnectionStatus["deviceInfo"]>;
}) {
  return (
    <section className="device-info-panel">
      <div className="panel-header">
        <h2>Device info</h2>
        <p>
          Returned from <code>getInfo()</code> after connect.
        </p>
      </div>
...
export function App() {
  ...
  const [syncResult, setSyncResult] = useState<SyncAttendanceResult | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    async function bootstrap() {
      try {
        ...
        // Wait while sidecar auto-connect / auto attendance sync is in progress.
        if (status.connecting || status.sync.inProgress) {
          setBusy(status.connecting ? "connect" : "sync");
          for (let i = 0; i < 120; i++) {
          ...
          setBusy(null);
        } else if (status.connected && savedConfig.webhookUrl?.trim()) {
          // Connect finished; attendance sync may start a moment later.
          setBusy("sync");
          for (let i = 0; i < 8; i++) {
            await new Promise((r) => setTimeout(r, 250));
            if (cancelled) return;
            status = await getStatus();
            if (status.sync.inProgress) break;
          }
          while (status.sync.inProgress) {
            await new Promise((r) => setTimeout(r, 250));
            if (cancelled) return;
            status = await getStatus();
          }
        ...
        if (status.connected) {
          const syncNote =
            status.webhook.enabled && status.webhook.passesForwarded > 0
              ? ` Attendance synced (${status.webhook.passesForwarded} records).`
              : status.webhook.enabled
                ? " Attendance sync finished."
                : "";
          setMessage(
            `Auto-connected to ${status.config?.ip ?? savedConfig.ip} on startup.${syncNote}`,
          );
  ...
  function updateField<K extends keyof ClientConfig>(
    key: K,
  ...
  return (
    <div className="app-shell">
      <header className="app-header">
        <div>
          ...
          <p className="subtitle">
            Configure your device and verify connectivity.
          </p>
        </div>
      ...
      {activeTab === "device" && status.deviceInfo ? (
        <DeviceInfoPanel info={status.deviceInfo} />
      ) : null}
            ...
            >
              ...
              <div className="field-grid">
                ...
                <label>
                  ...
                  <input
                    ...
                    onChange={(event) =>
                      updateField("timeout", Number(event.target.value))
                    }
                  />
                </label>
                <label>
                  ...
                  <input
                    ...
                    onChange={(event) =>
                      updateField("udpPort", Number(event.target.value))
                    }
                  />
                </label>
                <label>
                  ...
                  <input
                    ...
                    onChange={(event) =>
                      updateField("commKey", Number(event.target.value))
                    }
                  />
                </label>
              </div>
              ...
              <label>
                ...
                <input
                  ...
                  onChange={(event) =>
                    updateField("openDoorDelaySec", Number(event.target.value))
                  }
                />
              </label>
          ...
          <section className="panel">
            ...
            {testResult ? (
              <div className={`test-result ${testResult.ok ? "ok" : "fail"}`}>
                <strong>
                  {testResult.ok ? "Last test passed" : "Last test failed"}
                </strong>
                <span>
                  {testResult.ok
            ...
            {syncResult ? (
              <div className={`test-result ${syncResult.ok ? "ok" : "fail"}`}>
                ...
                <span>
                  {syncResult.sent} sent, {syncResult.failed} failed of{" "}
                  {syncResult.total} · {syncResult.durationMs} ms
                </span>
              </div>
            ) : null}
                  ...
                  <button
                    ...
                    onClick={() =>
                      runAction("sync", async () => {
                        ...
                        if (result.ok) {
                          setMessage(
                            `Synced ${result.sent} attendance records to webhook.`,
                          );
                        } else {
                          setError(
                ...
                <button
                  ...
                  onClick={() =>
                    runAction("connect", async () => {
                      ...
                      setStatus(next);
                      if (next.deviceInfo) {
                        const { userCounts, logCounts, logCapacity } =
                          next.deviceInfo;
                        setMessage(
